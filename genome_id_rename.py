...
def rename(args):
    ...
    f_out = open('%s.gff3' % args.prefix, 'w')
    id_change=open('%s.id.change' % args.prefix, 'w')
    seqid = None
    scaf_num=0
    genenum=0
    flag='no'
    for gene in db.features_of_type("gene",order_by=('seqid','start','end')):
        if gene.seqid != seqid and not re.match(r'scaffold',gene.seqid):
            genenum = 0
        if re.match(r'scaffold',gene.seqid) and flag=='no':
            genenum=0
            flag='yes'
        genenum += args.addnum
        seqid = gene.seqid
        
        genename = '{0}g{1:05}'.format(seqid2name[seqid],genenum)
        ...
        for t,mRNA in enumerate(db.children(gene,featuretype="mRNA",order_by=('seqid','start','end'))):
            ...
            id_change.write(mRNA.attributes["ID"][0]+'\t'+mrnaid+'\n')
